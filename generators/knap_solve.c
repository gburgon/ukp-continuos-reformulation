...
int main() {
    ...
    int *x = malloc(n * sizeof(int));   // solução
    ...
    for (int i = 0; i < n; i++) {
        int idx;
        fscanf(file, "%d %d %d", &idx, &p[i], &w[i]);
        x[i] = 0;   // inicializa solução com zeros
    }
    ...
    printf("Lido: n = %d, capacidade = %d\n", n, c);

    // ---- chama o solver ----
    long opt = minknap(n, p, w, x, c);

    printf("\n========== RESULTADO ==========\n");
    printf("Valor ótimo: %ld\n", opt);
